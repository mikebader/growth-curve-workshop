#### ANALYSIS EXAMPLE: NONLINEAR TRENDS ####
...
T <- length(unique(zillow.long$month))

## Analyze only the boom/bust months (Apr 2004 to Oct 2009)
boom_bust <- zillow.long[zillow.long$month%in%c(96:162),]
boom_bust$month <- boom_bust$month-96
T <- max(boom_bust$month)
...
m.ana <- lmer(lnvalue_ti ~ month + I(month^2/100) +(1 + month  |RegionID),data=boom_bust)
summary(m.ana)
...
## INTERPRET THE DATA
d.fit <- data.frame(month=c(0:T),month2=c(0:T)^2)
d.fit$best_fit <- matrix(c(rep(1,length(0:T)),d.fit$month,d.fit$month2),ncol=3) %*% matrix(m.ana.fe,ncol=1)
