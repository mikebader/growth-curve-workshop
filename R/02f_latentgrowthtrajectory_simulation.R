...
# Set the number of individuals and the number of observations for each individual
N.i.com <- 40
N.t <- 10
...
# Set stochastic parameters of the model. We are going to allow those to be the
# same across models because of our assumption of conditional independence
tau0    <- 10
tau1    <- 3
sigma   <- 2
...
plot(0,0,type='n',xlim=c(-1,10),ylim=c(min(unlist(f.df)),max(unlist(f.df))),ylab='y',xlab='time')
l <- sapply(f.df[1:N.i.com],function(data) lines(c(0:t.fin),data,lty=2,col='red'))
l <- sapply(f.df[-1:-N.i.com],function(data) lines(c(0:t.fin),data,lty=2,col='blue'))
...
summary(m1.hlme)
lines(c(0,t.fin),c(m1.hlme$best[3],m1.hlme$best[3]+t.fin*m1.hlme$best[5]),lty=2,lwd=3,col='red')
lines(c(0,t.fin),c(m1.hlme$best[2],m1.hlme$best[2]+t.fin*m1.hlme$best[4]),lty=2,lwd=3,col='blue')

# Now we estimate the growth mixture model that includes the individual-specific
# error terms
m2.hlme <-hlme(y~time,subject='i',random=~time,ng=2,mixture=~time,idiag=TRUE,data=df)
summary(m2.hlme)
lines(c(0,t.fin),c(m2.hlme$best[2],m2.hlme$best[2]+t.fin*m2.hlme$best[4]),lty=3,lwd=3,col='red')
lines(c(0,t.fin),c(m2.hlme$best[3],m2.hlme$best[3]+t.fin*m2.hlme$best[5]),lty=3,lwd=3,col='blue')
