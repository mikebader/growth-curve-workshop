...
## Real Home Value Trends in New York and Washington Metros
Now we can extend the example that we had used before to model the trend from New York to look at the real combined trend in home values between New York and Washington in 2015. 

The first thing that we need to do is get the data for both New York and Washington. 

```{r modeling-multiple-nydc-real-data}
zillow <- read.csv(price.sq.ft.url,header=TRUE)
varnames.2015 <- paste0("X2015.",sprintf("%02.0f",1:12))
nydc15 <- zillow[zillow$RegionName %in% c("New York, NY","Washington, DC"),
             c("RegionID","RegionName",varnames.2015)
             ]
nydc15.long <- reshape(nydc15,idvar="RegionID",timevar="t",direction="long",varying=varnames.2015)
nydc15.long <- mutate(nydc15.long
                      , t = t -1 
                      , price_t = X2015
                      , lnprice_t = log(price_t)
                      , RegionID = factor(RegionID,labels=c("ny","dc"))
                      )
nydc15.long
```

And since we know what the New York metro data look like, let's take a look at the D.C. data: 

```{r modeling-multiple-dc-plot}
dc.plt <- qplot(t,lnprice_t,data=nydc15.long[nydc15.long$RegionName=="Washington, DC",],colour = I(dc.color))
dc.plt
```

Okay, now let's create the separate trend models for New York and D.C.: 

```{r modeling-multiple-nydc-real-separate-lm}
ny.model <- lm(lnprice_t ~ t,data=nydc15.long[nydc15.long$RegionName=="New York, NY",])
summary(ny.model)
ny.coef <- ny.model$coefficients

dc.model <- lm(lnprice_t ~ t,data=nydc15.long[nydc15.long$RegionName=="Washington, DC",])
summary(dc.model)
dc.coef <- dc.model$coefficients

nydc.plt <- qplot(t,lnprice_t,data=nydc15.long,color=RegionID) +
                    scale_x_continuous(breaks=seq(0,11),labels=month.abb,minor_breaks=NULL) +
                    scale_colour_manual(values=c(ny.color,dc.color)) +
                    geom_abline(intercept=ny.coef[1],slope=ny.coef[2],colour=ny.color) +
                    geom_abline(intercept=dc.coef[1],slope=dc.coef[2],colour=dc.color) +
...
This time we didn't center time at its midpoint since it makes it a little easier to think about the intercept as the starting value. 

**Exercise:** I want to you to interpret the values of the model and explain what is going on in New York and in D.C. 

```{r modeling-multiple-nydc-real-lm}
nydc.model <- lm(lnprice_t ~ t, data=nydc15.long)
nydc.coef <- nydc.model$coefficients
summary(nydc.model)
nydc.plt + geom_abline(intercept=nydc.coef[1],slope=nydc.coef[2])
